...
def get_recent_messages(limit_minutes=10):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cutoff_time = (datetime.utcnow() - timedelta(minutes=limit_minutes)).isoformat()
    cursor.execute("SELECT timestamp, raw_data FROM packets WHERE timestamp >= ? ORDER BY timestamp DESC", (cutoff_time,))
    rows = cursor.fetchall()
    conn.close()
    return [(ts, msg) for ts, msg in rows]
